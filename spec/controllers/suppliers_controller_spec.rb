...
describe SuppliersController do
  ...
  def mock_supplier(stubs={})
    @mock_supplier ||= mock_model(Supplier, stubs)
  end

  describe "GET index" do
    it "assigns all suppliers as @suppliers" do
      Supplier.stub(:find).with(:all).and_return([mock_supplier])
      get :index
      assigns[:suppliers].should == [mock_supplier]
    end
  end

  describe "GET show" do
    it "assigns the requested supplier as @supplier" do
      Supplier.stub(:find).with("37").and_return(mock_supplier)
      get :show, :id => "37"
      assigns[:supplier].should equal(mock_supplier)
    end
  end

  describe "GET new" do
    it "assigns a new supplier as @supplier" do
      Supplier.stub(:new).and_return(mock_supplier)
      get :new
      assigns[:supplier].should equal(mock_supplier)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created supplier as @supplier" do
        Supplier.stub(:new).with({'these' => 'params'}).and_return(mock_supplier(:save => true))
        post :create, :supplier => {:these => 'params'}
        assigns[:supplier].should equal(mock_supplier)
      end

      it "redirects to the created supplier" do
        Supplier.stub(:new).and_return(mock_supplier(:save => true))
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved supplier as @supplier" do
        Supplier.stub(:new).with({'these' => 'params'}).and_return(mock_supplier(:save => false))
        post :create, :supplier => {:these => 'params'}
        assigns[:supplier].should equal(mock_supplier)
      end

      it "re-renders the 'new' template" do
        Supplier.stub(:new).and_return(mock_supplier(:save => false))
        post :create, :supplier => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested supplier" do
        Supplier.should_receive(:find).with("37").and_return(mock_supplier)
        mock_supplier.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :supplier => {:these => 'params'}
      end

      it "assigns the requested supplier as @supplier" do
        Supplier.stub(:find).and_return(mock_supplier(:update_attributes => true))
        put :update, :id => "1"
        assigns[:supplier].should equal(mock_supplier)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the supplier as @supplier" do
        Supplier.stub(:find).and_return(mock_supplier(:update_attributes => false))
        put :update, :id => "1"
        assigns[:supplier].should equal(mock_supplier)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the suppliers list" do
      Supplier.stub(:find).and_return(mock_supplier(:destroy => true))
      delete :destroy, :id => "1"
