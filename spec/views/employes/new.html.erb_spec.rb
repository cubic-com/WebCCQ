...
describe "/employes/new.html.erb" do
  include EmployesHelper

  before(:each) do
    assigns[:employe] = stub_model(Employe,
      :new_record? => true,
      ...
      :city => "value for city",
      :postal_code => "value for code_postal",
      :ssn => "value for ssn",
  ...
  it "renders new employe form" do
    ...
    response.should have_tag("form[action=?][method=post]", employes_path) do
